...
// You'll need to set up environment variables for email
...
app.post('/api/submit-preview-request', async (req, res) => {
  try {
    const {
    ...
    const emailTemplate = (content) => `
      ...
      <head>
        ...
        <style>
          body { font-family: -apple-system, BlinkMacSystemFont, 'Segoe UI', Arial, sans-serif; color: #2a2a2a; }
          ...
          .badge-group { display: flex; gap: 8px; flex-wrap: wrap; margin-top: 10px; }
          .badge { display: inline-block; background: #e8f0ed; color: #2e4a3e; padding: 6px 12px; border-radius: 20px; font-size: 12px; font-weight: 500; }
          .cta-button { display: inline-block; background: #2e4a3e; color: #fdfdfb; padding: 12px 24px; border-radius: 4px; text-decoration: none; font-weight: 500; margin-top: 15px; }
    ...
    const ownerContent = `
      <div class="section">
        ...
        <div class="info-row">
          ...
          <div class="info-value">${escapeHtml(businessType)}</div>
        </div>
        <div class="info-row">
          <div class="info-label">Email</div>
          <div class="info-value"><a href="mailto:${escapeHtml(email)}" style="color: #2e4a3e; text-decoration: none;">${escapeHtml(email)}</a></div>
        </div>
        <div class="info-row">
          <div class="info-label">Phone</div>
          <div class="info-value">${escapeHtml(phone || 'Not provided')}</div>
        </div>
        ${website ? `<div class="info-row"><div class="info-label">Website</div><div class="info-value"><a href="${escapeHtml(website)}" style="color: #2e4a3e; text-decoration: none;">${escapeHtml(website)}</a></div></div>` : ''}
      ...
      <div class="section">
        ...
        <div class="info-row">
          ...
          <div class="info-value"><strong>${escapeHtml(designStyle)}</strong></div>
        </div>
        <div class="info-row">
          <div class="info-label">Primary Color</div>
          <div class="info-value"><strong>${escapeHtml(colorPreference)}</strong></div>
        </div>
        <div class="info-row">
          <div class="info-label">Brand Tone</div>
          <div class="info-value"><strong>${escapeHtml(tone)}</strong></div>
        </div>
        <div style="margin-top: 15px;">
          <div style="font-weight: 500; color: #2e4a3e; margin-bottom: 8px;">Key Features</div>
          <div class="info-box">${escapeHtml(keyFeatures).replace(/\n/g, '<br>')}</div>
        </div>
      ...
      <div style="text-align: center; color: #708090; font-size: 12px;">
        <p>Received: ${new Date().toLocaleString()}</p>
      </div>
    `;

    // Generate filled-in Claude prompt
    ...
    const promptContent = `
      <div class="section">
        ...
        <div style="background: #1a2820; border-radius: 6px; padding: 24px; font-family: 'Courier New', Courier, monospace; font-size: 12px; color: #a8c5b8; line-height: 1.8; white-space: pre-wrap; word-break: break-word;">${escapeHtml(claudePrompt)}</div>
      </div>
    `;

    // Email to owner — client overview
    const ownerMailOptions = {
      from: process.env.EMAIL_USER,
      to: process.env.OWNER_EMAIL || 'sales@trikemedia.com',
      subject: `🦖 New Preview Request: ${companyName}`,
      html: emailTemplate(ownerContent)
    };

    // Email to owner — Claude prompt ready to use
    const promptMailOptions = {
      from: process.env.EMAIL_USER,
      to: process.env.OWNER_EMAIL || 'sales@trikemedia.com',
    ...
    const customerContent = `
      <div class="section">
        ...
        <p style="color: #708090; line-height: 1.6; margin-bottom: 15px;">
          Thanks for reaching out! We've received your preview request and our team is excited to get started.
        </p>
        ...
        <div style="background: #e8f0ed; border-left: 4px solid #2e4a3e; padding: 20px; border-radius: 4px; margin: 25px 0;">
          ...
          <p style="color: #2e4a3e; margin: 0; line-height: 1.6;">
            We'll build a custom website preview based on your design preferences. You'll receive a live link to your preview <strong>within 48 hours</strong>.
          </p>
        </div>
        ...
        <p style="color: #708090; line-height: 1.6; margin: 20px 0;">
          If you have any questions before your preview arrives, just reply to this email or reach out to us.
        </p>

        <p style="color: #708090; margin-top: 25px;">
          Best,<br>
          <strong>The Trike Media Team</strong>
        </p>
      </div>
    `;
    ...
    await transporter.sendMail(promptMailOptions);
    ...
    console.error('Error submitting preview request:', error.message);
...
app.get('/test-email', async (req, res) => {
  try {
    ...
    const testMail = {
      ...
      subject: 'Test Email from Trike Media',
...
// Build filled-in Claude prompt from form data
